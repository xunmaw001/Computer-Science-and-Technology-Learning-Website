            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">通知展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="tongzhiName" id="tongzhiName" placeholder="通知名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="tongzhiTypes" id="tongzhiTypes" lay-filter="tongzhiTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in tongzhiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                        ...
                        <div class="article-list">
                            <ul class="after">
                                <li v-for="(item,index) in dataList" v-bind:key="index">
                                    <a target="_self" :href="'../tongzhi/detail.jsp?id='+item.id">
                                        <img width="180" class="fl" :src="item.tongzhiPhoto" />
                                        <h5>{{item.tongzhiName}}</h5>
                                        <p class="time-sour">
                                            <span>发布时间：{{item.insertTime}} </span>
                                        </p>
                                        <p class="reamk">{{item.tongzhiContent|tongzhiContent}}...</p>
                                    </a>
                                </li>
                            </ul>
                            <div class="pages">
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 tongzhiContent 字段去除无用字符 如果字段长度大于60  就截取到60
            tongzhiContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#tongzhiName').val() != null && jquery('#tongzhiName').val() != "" && jquery('#tongzhiName').val() != "null" ) {
                param['tongzhiName'] = jquery('#tongzhiName').val();
            }
         
            var tongzhiTypes = $("#tongzhiTypes").find("option:selected").val();
            if (tongzhiTypes != null && tongzhiTypes != "" && tongzhiTypes != "null") {
                param['tongzhiTypes'] = tongzhiTypes;
            }
                                    // 获取列表数据
            http.request('tongzhi/list', 'get', param, function(res) {
