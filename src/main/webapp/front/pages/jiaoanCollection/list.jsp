            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">教案收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="jiaoanName" id="jiaoanName" placeholder="教案名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">教案类型</label>
                              <div class="layui-input-inline">
                                  <select name="jiaoanTypes" id="jiaoanTypes" lay-filter="jiaoanTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in jiaoanTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../jiaoan/detail.jsp?id='+item.jiaoanId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.jiaoanPhoto?item.jiaoanPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../jiaoan/detail.jsp?id='+item.jiaoanId)"><span>{{item.jiaoanName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.jiaoanNewMoney"><span style="color: red;">{{item.jiaoanNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteJiaoanCollection(item.id)">删除 {{item.jiaoanName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 jiaoanCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            jiaoanCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteJiaoanCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('jiaoanCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#jiaoanName').val() != null && jquery('#jiaoanName').val() != "" && jquery('#jiaoanName').val() != "null" ) {
                param['jiaoanName'] = jquery('#jiaoanName').val();
            }
                     
            var jiaoanTypes = $("#jiaoanTypes").find("option:selected").val();
            if (jiaoanTypes != null && jiaoanTypes != "" && jiaoanTypes != "null") {
                            param['jiaoanTypes'] = jiaoanTypes;
                        }
                                                                                
            // 当前表
                                                    // 获取列表数据
            http.request('jiaoanCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function jiaoanTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jiaoan_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.jiaoanTypesList = res.data.list;
               }
           });
       }
