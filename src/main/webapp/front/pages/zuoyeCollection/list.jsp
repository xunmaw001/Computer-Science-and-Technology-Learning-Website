            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">作业收藏展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <%--<div class="layui-inline" style="margin-bottom: 10px;">--%>
                              <%--<label class="layui-form-label">作业截止时间</label>--%>
                              <%--<div class="layui-input-inline">--%>
                                  <%--<input type="text" name="zuoyeEndTimeStart" id="zuoyeEndTimeStart"  lay-skin="date"   placeholder="作业截止时间开始" autocomplete="off" class="layui-input">--%>
                              <%--</div>--%>
                          <%--</div>--%>
                          <%--<div class="layui-inline" style="margin-bottom: 10px;">--%>
                              <%--<div class="layui-input-inline" style="margin-top: -29px">--%>
                                  <%--<input type="text" name="zuoyeEndTimeEnd"   id="zuoyeEndTimeEnd"    lay-skin="date"   placeholder="作业截止时间结束" autocomplete="off" class="layui-input">--%>
                              <%--</div>--%>
                          <%--</div>--%>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../zuoye/detail.jsp?id='+item.zuoyeId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.zuoyePhoto?item.zuoyePhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../zuoye/detail.jsp?id='+item.zuoyeId)"><span>{{item.zuoyeName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.zuoyeNewMoney"><span style="color: red;">{{item.zuoyeNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteZuoyeCollection(item.id)">删除 {{item.zuoyeName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 zuoyeCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            zuoyeCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteZuoyeCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('zuoyeCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            var zuoyeTypes = $("#zuoyeTypes").find("option:selected").val();
            if (zuoyeTypes != null && zuoyeTypes != "" && zuoyeTypes != "null") {
                            param['zuoyeTypes'] = zuoyeTypes;
                        }
                                         
            if (jquery('#zuoyeEndTimeStart').val() != null && jquery('#zuoyeEndTimeStart').val() != "" && jquery('#zuoyeEndTimeStart').val() != "null" ) {
                param['zuoyeEndTimeStart'] = jquery('#zuoyeEndTimeStart').val();
            }
            if (jquery('#zuoyeEndTimeEnd').val() != null && jquery('#zuoyeEndTimeEnd').val() != "" && jquery('#zuoyeEndTimeEnd').val() != "null" ) {
                param['zuoyeEndTimeEnd'] = jquery('#zuoyeEndTimeEnd').val();
            }
                                                            
            // 当前表
                                                    // 获取列表数据
            http.request('zuoyeCollection/list', 'get', param, function(res) {
